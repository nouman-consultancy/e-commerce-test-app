...
export default async function OrderDetailPage({
  ...
  const order = await serverFetch<Order>(`/orders/${id}`, session.user.accessToken).catch(() => null);
      ...
      <div className="bg-white rounded-lg border border-gray-200 p-5 mb-4">
        <div className="grid grid-cols-2 gap-4 text-sm">
          <div>
            ...
            <p className="font-mono font-semibold text-gray-900 mt-0.5">#{shortId(order.id)}</p>
          </div>
          <div>
            <p className="text-gray-500">Date Placed</p>
            <p className="font-semibold text-gray-900 mt-0.5">{formatDate(order.createdAt)}</p>
          </div>
          <div>
            <p className="text-gray-500">Payment Ref</p>
            <p className="font-mono text-xs text-gray-700 mt-0.5 break-all">{order.paymentRef}</p>
          </div>
